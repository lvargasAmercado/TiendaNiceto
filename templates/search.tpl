	...
	<div class="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4 fade-in-up">
		<div>
			<nav class="flex text-xs text-gray-500 mb-2 gap-2">
				<a href="{{ store.url }}" class="hover:text-black">Inicio</a>
				<span>/</span>
			...
			{% if products %}
				<p class="text-sm text-gray-500 mt-2">Mostrando resultados para "{{ query }}"</p>
			{% endif %}
		</div>
		...
		<div class="md:hidden w-full relative">
			...
			<form action="{{ store.search_url }}" method="get">
				<input type="text" name="q" placeholder="Buscar..." value="{{ query }}" class="w-full bg-white border border-gray-200 rounded-lg py-2 pl-10 pr-4 text-sm">
			</form>
		</div>
		...
		<aside class="w-full lg:w-64 flex-shrink-0 fade-in-up">
			<div class="sticky top-24">
				...
				<div class="flex lg:flex-col gap-2 overflow-x-auto hide-scrollbar pb-2 lg:pb-0">
					<a href="{{ store.products_url }}" class="filter-btn whitespace-nowrap px-4 py-2 lg:px-0 lg:py-1.5 lg:pl-3 text-sm rounded-full lg:rounded-md border border-gray-200 lg:border-transparent lg:text-left text-gray-500 hover:text-black hover:bg-gray-50 transition-colors">Ver Todo</a>
					{% for filter_category in filter_categories %}
						<a href="{{ filter_category.url }}" class="filter-btn whitespace-nowrap px-4 py-2 lg:px-0 lg:py-1.5 lg:pl-3 text-sm rounded-full lg:rounded-md border border-gray-200 lg:border-transparent lg:text-left text-gray-500 hover:text-black hover:bg-gray-50 transition-colors">{{ filter_category.name }}</a>
					{% endfor %}
				</div>

				{% if product_filters is not empty and search_filter %}
